#ifndef MULTICANONIC_CPN_C
#define MULTICANONIC_CPN_C
...
void real_main(char const * const input_file_name) 
{
	CPN_Conf *conf;
	CPN_Conf aux_conf;
	CPN_Param param;	
	...
	FILE *datafilep, *swaptrackfilep, *topofilep, *multicanonic_acc_filep;
	double *grid;
	int i;
	...
	// open multicanonic acceptances file
	...
	// initialize topo_charge geometric definition
	init_topo_charge(conf, &geo, &param);
	...
	{
		// perform a single step of parallel tempering with hierarchic updates and print state of replica swap with multicanonical approach
		multicanonic_parallel_tempering_with_hierarchic_update(conf, most_update, &swap_counter, &param, &geo, &aux_conf, grid, multicanonic_acc_filep, &rng_state);
		...
		if ( (conf[0].update_index) % param.d_measevery == 0)
		{
			...
			refresh_topo_charge_replica(conf, &geo, &param);
		}
	...
	print_simulation_details_multicanonic_cpn(input_file_name, &param, &start_date, &finish_date, start_time, finish_time);
...
int main (int argc, char **argv)
{
	...
	if(argc != 2)
	{
		...
		printf("Description: lattice simulations of 2d CP^{N-1} models topology via parallel tempering + multicanonic approach\n\n");
		printf("Author: Claudio Bonanno\n");
		printf("Other contributors: Mario Berni, Francesca Margari, Davide Vadacchino\n");
		printf("Bug report: %s\n", PACKAGE_BUGREPORT);
		printf("\nCompiled with N = %i\n", N);
